...
function messageHandler(topic, message, protocol) {
    console.log(topic, message, protocol);
    if (topic[0] !== '/') {
        topic = '/' + topic;
    }
 ...
 */
function opcuaMessageHandler(deviceId, mapping, variableValue) {
    const topic = config.getConfig().iota.defaultResource + '/' + deviceId + '/attrs/' + mapping.ocb_id;
