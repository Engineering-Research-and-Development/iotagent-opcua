...
async function writeValueToOPCUANode(opcuaNodeId, value) {
    if (!the_session) {
    ...
    const statusCode = await the_session.write(nodesToWrite);
    if (statusCode == opcua.StatusCodes.Good) {
        config.getLogger().info(context, opcuaNodeId + ' write variable -> ' + value);
        return true;
    } else {
        config.getLogger().error(context, opcuaNodeId + ' write failed: ' + statusCode._description);
        return { name: statusCode._name, message: statusCode._description };
    }
}
...
async function readValueTypeFromOPCUANode(opcuaNodeId) {
    ...
    const dataType = await the_session.readVariableValue(opcuaNodeId);
    if (dataType.value == null) {
        return null;
    } else {
        config.getLogger().info(context, opcuaNodeId + ' read variable type -> ' + dataType.toString());
...
exports.writeValueToOPCUANode = writeValueToOPCUANode;
exports.start = start;
exports.stop = stop;
