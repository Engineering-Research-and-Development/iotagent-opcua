 ...
 */
async function executeCommand(apiKey, device, attribute) {
    if (!the_session) {
        config.getLogger().error(context, 'No Connection to the OPC UA Server');
        return null;
    }
    ...
    foundMapping.inputArguments[0]['value'] = attribute.value;

    var methodsToCall = [];
    methodsToCall.push({
        objectId: '' + foundMapping.object_id,
        methodId: '' + foundMapping.opcua_id,
...
async function executeQuery(apiKey, device, attribute, callback) {
    ...
    var lazyObject = lazySet.find((lazyAttribute) => lazyAttribute.name === attribute);
    if (!lazyObject) {
...
async function start() {
    ...
    console.log('certificateFolder', certificateFolder);
    ...
    console.log('resolvedCertificateFilePath', resolvedCertificateFilePath);
    var resolvedPrivateKeyFilePath = path.resolve(privateKeyFile).replace(/\\/g, '/');
    console.log('resolvedPrivateKeyFilePath', resolvedPrivateKeyFilePath);
    ...
    console.log('opcUAClientOptions', opcUAClientOptions);
