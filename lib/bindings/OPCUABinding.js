            ...
            if (results && results.length > 0) {
                ...
                if (result.statusCode == opcua.StatusCodes.Good) {
                    const itemToMonitor = {
                        ...
                        attributeId: mapping.isEvent ? opcua.AttributeIds.EventNotifier  : opcua.AttributeIds.Value
                    };
                    ...
                    try {
                        ...
                        function monitorEvent(events) { //Variant[]
                            /* istanbul ignore else */
                            for(let i=0;i<events.length;i++) {
                                config.getLogger().info(context, fields[i],"=", events[i].toString());
                                commonBindings.opcuaEventHandler(opcuaServerConfigContext.id, mapping, dataValue.value.value, dataValue.sourceTimestamp);
                            }
                        }
                        function monitorValue(dataValue) {
                            /* istanbul ignore else */
                        ...
                        monitoredItem.on('changed', mapping.isEvent ? monitorEvent(events) : monitorValue(dataValue));
