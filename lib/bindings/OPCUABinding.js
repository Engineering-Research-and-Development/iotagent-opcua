                            ...
                            monitoredItem.on('changed', function (events) {
                                ...
                                fields.forEach(function (field, index) {
                                    if (field.type == 'ByteString') {
                                        variableValue += events[index] && events[index].value ? Buffer(events[index].value).toString('hex') : 'N/A';
                                ...
                                const forbiddenCharacters = config.getConfig().iota.extendedForbiddenCharacters ? [...config.getConfig().iota.extendedForbiddenCharacters, ...FORBIDDEN_NGSI_CHARS] : FORBIDDEN_NGSI_CHARS;
                                config.getLogger().info(context, 'Using forbidden characters set: ' + JSON.stringify(forbiddenCharacters));
                                variableValue = [...variableValue].map((char) => (forbiddenCharacters.includes(char) ? '' : char)).join('');
...
async function start() {
    ...
    async.series([connectToClient, initProvisioning, createSession, createSubscription, startMonitoring], function (err) {
        /* istanbul ignore if */
        if (err) {
...
async function initProvisioning() {
    try {
        if (config.getConfig().iota.autoprovision) {
