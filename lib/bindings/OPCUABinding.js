        ...
        opcuaServerConfigContext.mappings.forEach(async function (mapping) {
            ...
            if (results && results.length > 0) {
                ...
                if (result.statusCode == opcua.StatusCodes.Good) {
                    if(config.getConfig().iota.events.find(obj => obj.opcua_id === mapping.opcua_id )){
                        const itemToMonitor = {
                        ...
                        try {
                            ...
                            monitoredItem.on('changed', function(events) { //Variant[]
                                var variableValue = "";
                                fields.forEach(function(field,index){
                                    if(field.type == "ByteString"){
                                        variableValue += Buffer(events[index].value).toString('hex');
                                    }else if(field.type == "NodeId"){
                                        variableValue += events[index].value.value;
                                    }else if(field.type == "String" || field.type == "UInt16"){
                                        variableValue += events[index].value;
                                    }else if(field.type == "DateTime"){
                                        variableValue += moment(events[index].value).valueOf();
                                    }else if(field.type == "LocalizedText"){
                                        variableValue += events[index].value.text;
                                    }
                                    variableValue += " | "
                                }); 
                                variableValue = variableValue.slice(0,-3);
                                variableValue = [...variableValue].map(char => FORBIDDEN_NGSI_CHARS.includes(char) ? "" : char).join('');
