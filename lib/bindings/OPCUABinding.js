                    ...
                    if(mapping.isEvent){
                        ...
                        try {
                            ...
                            monitoredItem.on('changed', function(events) { //Variant[]
                                ...
                                const message = events[0].value.text + " | " + "Severity:" + events[1].value + " | " + "Time: " + Math.floor(new Date(events[2].value).getTime() / 1000);
                                config.getLogger().info(context, opcua_id + ' event triggered -> ' + events[0].value.text);
                                commonBindings.opcuaMessageHandler(opcuaServerConfigContext.id, mapping, message, events[2].value);
                            });
                        } catch (err) {
                            /* istanbul ignore next */
                            throw new Error(err);
