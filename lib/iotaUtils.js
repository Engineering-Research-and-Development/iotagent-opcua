...
const groupConfig = iotAgentLib.groupConfig;
...
function retrieveDevice(deviceId, apiKey, transport, callback) {
    if (apiKey === config.getConfig().defaultKey) {
        ...
        async.waterfall(
            [apply(iotAgentLib.getConfigurationSilently, config.getConfig().iota.defaultResource || '', apiKey), apply(createGroupIfNotExists), apply(findOrCreate, deviceId, transport), apply(iotAgentLib.mergeDeviceWithConfiguration, ['lazy', 'active', 'staticAttributes', 'commands', 'subscriptions'], [null, null, [], [], [], [], []])],
            callback
        );
    }
}
...
/**
 * Create a MongoGroup if not existing, given the apiKey and resource path
 *
 *
...
function createGroupIfNotExists() {
    return new Promise((resolve, reject) => {
        var parameters = {
            services: [
                {
                    apikey: `${config.getConfig.iot.defaultKey}`,
                    cbroker: `http://${config.getConfig.contextBroker.host}:${config.getConfig.contextBroker.port}`,
                    entity_type: `${config.getConfig.iot.defaultType}`,
                    resource: `${config.getConfig.iot.defaultResource}`
                }
            ]
        };
