    ...
    createGroupIfNotExists()
        .then(() => {
            if (apiKey === config.getConfig().defaultKey) {
                ...
                async.waterfall([apply(iotAgentLib.getConfigurationSilently, config.getConfig().iota.defaultResource || '', apiKey), apply(findOrCreate, deviceId, transport), apply(iotAgentLib.mergeDeviceWithConfiguration, ['lazy', 'active', 'staticAttributes', 'commands', 'subscriptions'], [null, null, [], [], [], [], []])], callback);
            }
        })
        .catch((err) => {
...
function createGroupIfNotExists() {
    return new Promise((resolve, reject) => {
        ...
        axios
            .post('http://localhost:4041/iot/services', data, {
                headers: {
