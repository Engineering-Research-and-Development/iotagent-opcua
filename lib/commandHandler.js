                ...
                if (error) {
                    ...
                    async.series(funcList, function (err, results) {
                        if (err) {
                            ...
                            updateStatus(device, results, callback);
                        }
                    });
                }
            });
...
function updateStatus(device, results, statusCallback) {
    function updateAttributeStatus(result, innerCallback) {
        iotAgentLib.setCommandResult(device.name, device.type, device.apikey, result.attributeName, result.resultValue, result.statusCode === 0 ? 'OK' : 'ERROR', device, innerCallback);
    }

    async.map(results, updateAttributeStatus, statusCallback);
}
