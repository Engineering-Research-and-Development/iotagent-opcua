 ...
 * @return {string}                 Returns a serialized payload
 */
...
function commandHandler(id, type, service, subservice, attributes, callback) {
    ...
    iotAgentLib.getDeviceByNameAndType(id, type, service, subservice, function (error, device) {
        if (error) {
            config.getLogger().error(context, "COMMAND-001: Command execution could not be handled, as device for entity [%s] [%s] wasn't found", id, type);
            callback(error);
        } else {
            iotaUtils.getEffectiveApiKey(device.service, device.subservice, device, function (error, apiKey) {
                if (error) {
                    ...
                    const funcList = attributes.map(generateCommandExecution.bind(null, apiKey, device)).reduce(concat, []);
                    //funcList.push(updateStatus.bind(null, device, attributes));
                    async.series(funcList, function (err, results) {
                        if (err) {
                            config.getLogger().error(context, 'COMMAND-003: Command execution could not be handled, [%s]', err);
                            callback(err);
                        } else {
                            const statusFuncList = results.map(updateStatus.bind(null, device)).reduce(concat, []);
                            async.series(statusFuncList, function (err, results) {
...
async function updateStatus(device, output) {
    return await iotAgentLib.setCommandResult(device.name, device.type, device.apikey, output.attributeName, output.outputValue, output.statusCode === 0 ? 'OK' : 'ERROR', device, (err) => console.log(err));
}
