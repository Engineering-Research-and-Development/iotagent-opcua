            ...
            subscription
                ...
                .on('terminated', function(err) {
                    if (err) {
                        logger.error(
                            logContext,
                            'could not terminate subscription: ' + subscription.subscriptionId + ''.red.bold
                        );
            ...
            monitoredItem.on('changed', function(dataValue) {
                ...
                if (variableValue == null) {
                    ...
                    logger.info(
                        ...
                        ' value has changed to ' + variableValue + ''.bold.yellow
                    );
                                    ...
                                    function(callback) {
                                        iotAgentLib.register(device, function(err) {
                                            if (err) {
                                                ...
                                                logger.error(
                                                    logContext,
                                                    'could not register OCB context ' + context.id + ''.red.bold
                                                );
                                                logger.info(logContext, JSON.stringify(err).red.bold);
            ...
            async.series([
                ...
                function(callback) {
                    ...
                    if (result.count != 0) {
                        var attr = [
                            {
                                name: commandListAllDevices[0].name,
                                            ...
                                            iotAgentLib.getDevice(
                                                ...
                                                function(err, device) {
                                                    if (err) {
                                                        logger.error(
                                                            logContext,
                                                            'could not find the OCB context ' + context.id + ''.red.bold
                                                        );
                                                        logger.info(logContext, JSON.stringify(err).red.bold);
