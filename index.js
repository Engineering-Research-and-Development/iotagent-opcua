try{
  ...
  var qCh = require('./iot_agent_modules/run/queryContextHandler');
  var cCh = require('./iot_agent_modules/run/commandContextHandler');
  ...
  .alias("b", "browse")
  .example("run with autoConfig")
  .example("node index.js  -e opc.tcp://address:port -a ")
  .example("run without autoConfig ")
  .example("node index.js  -e opc.tcp://address:port ")
  ...
  if (doAuto){
    ...
    try {
      var child = exec('java -jar mapping_tool.jar  -e '+endpointUrl+' -f config.properties', function(err, stdout, stderr) {
